...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow p-8">
        <h1 class="text-2xl font-bold text-gray-900 mb-2">Edit Sport</h1>

        <!-- Info Box -->
        <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p class="text-sm text-blue-900">
                <span class="font-medium">Editing:</span> {{ $sport->name }}
                @if($sport->status === 'upcoming')
                    <span class="inline-block ml-2 bg-blue-100 text-blue-800 text-xs font-semibold px-2 py-1 rounded">Upcoming</span>
                @elseif($sport->status === 'ongoing')
                    <span class="inline-block ml-2 bg-green-100 text-green-800 text-xs font-semibold px-2 py-1 rounded">Ongoing</span>
                @else
                    <span class="inline-block ml-2 bg-gray-100 text-gray-800 text-xs font-semibold px-2 py-1 rounded">Completed</span>
                @endif
            </p>
        </div>

        <form action="{{ route('tenant.sports.update', [$university->slug, $sport]) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $sport->name) }}" required placeholder="e.g. Basketball, Volleyball" class="w-full px-4 py-2 border @error('name') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                @error('name')
            ...
            <div>
                ...
                <select name="category" required class="w-full px-4 py-2 border @error('category') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    <option value="team" @selected(old('category', $sport->category) === 'team')>Team Sport</option>
                    <option value="individual" @selected(old('category', $sport->category) === 'individual')>Individual Sport</option>
                </select>
            ...
            <!-- Bracket Type -->
            <div x-data="{ bracketType: '{{ old('bracket_type', $sport->bracket_type) }}' }">
                ...
                <select name="bracket_type" x-model="bracketType" required class="w-full px-4 py-2 border @error('bracket_type') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    <option value="single_elimination" @selected(old('bracket_type', $sport->bracket_type) === 'single_elimination')>Single Elimination</option>
                    <option value="double_elimination" @selected(old('bracket_type', $sport->bracket_type) === 'double_elimination')>Double Elimination</option>
                    <option value="round_robin" @selected(old('bracket_type', $sport->bracket_type) === 'round_robin')>Round Robin</option>
                </select>
            ...
            <div>
                ...
                <select name="facilitator_id" class="w-full px-4 py-2 border @error('facilitator_id') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    @foreach($facilitators as $facilitator)
                        <option value="{{ $facilitator->id }}" @selected(old('facilitator_id', $sport->facilitator_id) == $facilitator->id)>
                            {{ $facilitator->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="status" required class="w-full px-4 py-2 border @error('status') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    <option value="upcoming" @selected(old('status', $sport->status) === 'upcoming')>Upcoming</option>
                    <option value="ongoing" @selected(old('status', $sport->status) === 'ongoing')>Ongoing</option>
                    <option value="completed" @selected(old('status', $sport->status) === 'completed')>Completed</option>
                </select>
                @error('status')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
