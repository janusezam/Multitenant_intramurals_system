...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow p-8">
        ...
        <form action="{{ route('admin.universities.store') }}" method="POST" class="space-y-6">
            @csrf

            <!-- University Name -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">University Name</label>
                <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-4 py-2 border @error('name') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                @error('name')
            ...
            <div>
                ...
                <input type="text" name="slug" value="{{ old('slug') }}" required x-data x-model="slug" class="w-full px-4 py-2 border @error('slug') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" required class="w-full px-4 py-2 border @error('email') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                @error('email')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="date" name="plan_expires_at" value="{{ old('plan_expires_at') }}" required class="w-full px-4 py-2 border @error('plan_expires_at') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
